...
export async function recomputeDerived(client, restDays = new Set()) {
  ...
  // Bedtime as minutes past 18:00, so a 01:00 bedtime sorts after 23:00
  // instead of wrapping to the smallest value of the night.
  const bedMinutes = (ts) => {
    if (!ts) return null;
    const m = /T(\d{2}):(\d{2})/.exec(ts);
    if (!m) return null;
    const mins = Number(m[1]) * 60 + Number(m[2]);
    return mins < 12 * 60 ? mins + 12 * 60 : mins - 18 * 60;
  };
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Trailing 90-day baseline, this day's own value left out.
    const window = bedSeries.slice(Math.max(0, i - 90), i);
    const base = median(window);
    const deviation = bed != null && base != null ? bed - base : null;

    const bedHour = bed != null ? (bed + 18 * 60) / 60 : null;
    const gap = (hour) =>
