...
export const CHECKIN_SCHEMA = {
  ...
  properties: {
    ...
    meals_count: int('How many separate eating occasions today. His plan has five. Only if he said a number or listed them.'),
    water_l: num('Litres of water today. "two and a half" is 2.5, "8 glasses" is 2. null if not said.'),
    ...
    cigarettes: num('Cigarettes smoked today. Explicitly none is 0.'),
    last_cigarette_hour: num('Decimal hour of the last cigarette.'),
    food_text: str('Anything said about food beyond timing and size.'),
    weight_kg: num('Body weight in kg, only if he stated one. Asked weekly, in the morning.'),
...
CONTEXT: this answers the MORNING mail. Its main question is morning energy
1-5, so a bare single digit 1-5 is energy_am. Never fill energy_pm or focus
from it. On Sundays it also asks for body weight, so a number near 60-120
with a decimal, or one written with kg, is weight_kg and not a score.`,
  evening: `

CONTEXT: this answers the EVENING questionnaire below, about the day just
finished. Any unqualified "energy"/"אנרגיה" score is energy_pm, never energy_am.
Never fill energy_am from an evening reply.

Answers are often POSITIONAL and terse: a bare number near a topic is answering
that numbered item. "אזור 2" alongside a remark about work is item 4, so
work_stress 2. The questions are given here so the mapping does not depend on
them being quoted back in the reply:

1. אימון היום? (סוג / שעת התחלה / דקות / מאמץ 1-10, או "לא")
   -> workout_type, workout_start_hour, workout_duration_min, workout_rpe
2. אוכל: כמה ארוחות מתוך 5, שעת האחרונה, כמה כבדה, וכמה ליטר מים
   -> meals_count, last_meal_hour, meal_size, water_l
3. אלכוהול (מנות + שעה), קפאין (כוסות + שעה), סיגריות (כמה + שעה אחרונה)
   -> alcohol_units, last_drink_hour, caffeine_cups, last_caffeine_hour,
      cigarettes, last_cigarette_hour
4. לחץ בעבודה היום 1-5. קרה משהו חריג?  -> work_stress, acute_event, acute_note
5. אנרגיה וריכוז אחרי הצהריים, 1-5 כל אחד  -> energy_pm, focus
6. משהו לא שגרתי?  -> acute_event, acute_note, notes

He often reports YESTERDAY as well as today ("אתמול עשיתי אימון כוח"). Every
numeric field describes TODAY only. Anything about another day goes in notes,
never into a field.`,
};
...
export function sanitize(f) {
  ...
  return {
    ...
    // The plan has five slots plus water on waking; more than ten eating
    // occasions or more than eight litres is a misparse, not a day.
    meals_count: f.meals_count == null || f.meals_count < 0 || f.meals_count > 10
      ? null : f.meals_count,
    water_l: f.water_l == null || f.water_l < 0 || f.water_l > 8 ? null : f.water_l,
    ...
    cigarettes: f.cigarettes == null || f.cigarettes < 0 || f.cigarettes > 60
      ? null : f.cigarettes,
    last_cigarette_hour: hour(f.last_cigarette_hour),
    food_text: f.food_text || null,
    // A plausible adult range. Anything outside it is a misparse, not a weight.
    weight_kg: f.weight_kg == null || f.weight_kg < 35 || f.weight_kg > 250 ? null : f.weight_kg,
    notes: f.notes || null,
  };
}
