...
/**
 * Which day the questions are about, stated in the body and not only in the
 * subject. A reply written the next morning against a subject line that has
 * scrolled away files the answers to the wrong date, and the whole system is
 * built on matching an answer to the night the ring measured.
 */
export function dayLine(date) {
  return `השאלות האלה על ${hebrewDate(date)}.\n\n`;
}
...
/**
 * Weight is asked weekly, on Sundays, in the morning.
 *
 * Body composition is one of the three stated goals and Oura measures nothing
 * relevant to it, so without this the goal has no outcome at all and every
 * lever tested against it is untestable forever. Weekly rather than daily
 * because day-to-day weight is mostly water and asking daily is how a
 * questionnaire dies.
 */
export const WEIGHT_DAY = 0; // Sunday

export function morningEmail({ date, daysStale, lastDate, missedDate }) {
  const askWeight = new Date(date + 'T00:00:00Z').getUTCDay() === WEIGHT_DAY;
  return {
    subject: askWeight
      ? `אנרגיה ומשקל · ${hebrewDate(date)}`
      : `אנרגיה הבוקר? · ${hebrewDate(date)}`,
    body:
      dayLine(date) +
      stalenessBlock(daysStale, lastDate) +
      missedBlock(missedDate) +
      (askWeight
        ? '1. אנרגיה הבוקר, 1-5?\n2. משקל הבוקר בק"ג? (אם נשקלת)\n\n(שני מספרים, זהו)\n'
        : 'אנרגיה הבוקר, 1-5?\n\n(מספר אחד בתשובה, זהו)\n'),
  };
}
...
export function eveningEmail({ date }) {
  return {
    ...
    body: dayLine(date) + [
      ...
      '2. אוכל: כמה ארוחות מתוך 5 שבתוכנית, באיזו שעה סיימת את האחרונה,',
      '   כמה כבדה (כבדה / בינונית / קלה), וכמה ליטר מים.',
      '3. אלכוהול: כמה מנות ובאיזו שעה האחרונה.',
      '   קפאין: כמה כוסות ובאיזו שעה האחרונה.',
      '   סיגריות: כמה היום ובאיזו שעה האחרונה.',
...
export const GAP_QUESTIONS = {
  ...
  meals_count: 'כמה ארוחות אכלת היום, מתוך 5 שבתוכנית?',
  water_l: 'כמה ליטר מים שתית היום?',
  ...
  last_cigarette_hour: 'באיזו שעה הייתה הסיגריה האחרונה?',
...
export const GAP_PRIORITY = [
  'last_meal_hour', 'water_l', 'meals_count', 'last_caffeine_hour', 'last_drink_hour',
  'last_cigarette_hour',
...
export function findGaps(fields) {
  ...
  for (const f of GAP_PRIORITY) {
    ...
    if (f === 'last_cigarette_hour' && !fields.cigarettes) continue;
